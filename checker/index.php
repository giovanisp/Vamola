...
// Cross Site Request Forgery security by Pierluigi Tassi

if(!isset($_SESSION['once']))
{
    $_SESSION['once'] = md5(uniqid(rand(), true));
}

function check_csrf($session, $post)
{
    if ( !empty($session) && !empty($post) && $session == $post)
    {
        return true;
    }
...
{
    // CSRF security by Pierluigi Tassi
    if(false == check_csrf($_SESSION['once'], $_POST['once']))
    {
        $msg->addError('INVALID_REQUEST');
    }
...
if ($_POST["validate_file"])
{
    ...
    if(false == check_csrf($_SESSION['once'], $_POST['once']))
    {
        $msg->addError('INVALID_SESSION');
    }

    // MimeType security by Pierluigi Tassi
    if(!isset($_FILES['uploadfile']['type']) || $_FILES['uploadfile']['type'] <> 'text/html' )
    {
        $msg->addError('INVALID_FORMAT');
    }
