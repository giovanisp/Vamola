...
?>
	...
	<?php foreach ($checks_array as $check_row) { ?>
		<tr>
			...
			<td><span class="msg"><a target="info-window" href="<?php echo AC_BASE_HREF; ?>checker/suggestion.php?id=<?php echo $check_row["check_id"]; ?>" onclick="AChecker.popup('<?php echo AC_BASE_HREF; ?>checker/suggestion.php?id=<?php echo $check_row["check_id"]; ?>'); return false;"><?php echo htmlspecialchars(_AC($check_row['name'])); ?></a></span></td>
...
<?php
		...
		if (is_array($named_subgroups))
		{
			...
			{
?>
	<h4><?php echo _AC($subgroup['name']);?></h4>
<?php 
