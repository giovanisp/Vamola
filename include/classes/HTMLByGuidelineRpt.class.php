...
';
	...
	var $html_tr_header =
'           <div class="gd_th">
             <span width="5%">{PASS_TEXT}<br /><input type="checkbox" class="AC_selectAllCheckBox" id="selectall_{CHECK_ID}" name="selectall_{CHECK_ID}" title="{SELECT_ALL_TEXT}" /></span>
             <span width="95%"><label for="selectall_{CHECK_ID}">{SELECT_ALL_TEXT}</label></span>
           </div>
';

	var $html_tr_with_decision =
'           <div {ROW_SELECTED}>
             <span width="5%">{CHECKBOX}</span>
             <span width="95%" class="AC_problem_detail">{PROBLEM_DETAIL}</span>
           </div>
';
...
'        <span class="err_type"><img id="msg_icon_{LINE_NUMBER}_{COL_NUMBER}_{CHECK_ID}" src="{BASE_HREF}images/{IMG_SRC}" alt="{IMG_TYPE}" title="{IMG_TYPE}" width="15" height="15" /></span>
         ...
         <div><code class="input">{HTML_CODE}</code></div>
         {IMAGE}
         <p class="helpwanted">
         </p>
         {CSS_CODE}
';
	...
	var $html_repair = 
'         <span >{REPAIR_LABEL}:> </span>{REPAIR_DETAIL}
';
	
	var $html_question = 
'         <div>
           <div>{QUESTION_LABEL}:<span>{QUESTION}</span></div>
           <div>{PASS_LABEL}:<span>{PASS_ANSWER}</span></div>
           <div>{FAIL_LABEL}:<span>{FAIL_ANSWER}</span></div>
         </div>
';
...
'  <div>
    <span >
      <input type="button" value="{LABEL_MAKE_DECISION}" id="AC_btn_make_decision_{SUBGROUP_ID}" /></span>
...
';
	...
	var $html_congrats =
'<p><img alt="{CONGRATS_ALT}" src="/images/feedback.gif" />{CONGRATS_TEXT}</p>
';
